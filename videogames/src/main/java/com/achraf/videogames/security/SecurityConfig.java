...
public class SecurityConfig {
    ...
    public InMemoryUserDetailsManager inMemoryUserDetailsManager(PasswordEncoder passwordEncoder) {
        return new InMemoryUserDetailsManager(
                ...
                User.withUsername("achraf").password(passwordEncoder.encode("123")).roles("USER","AGENT").build(),
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeRequests().requestMatchers("/saveVideogame").hasAnyRole("ADMIN","AGENT");
        httpSecurity.authorizeRequests().requestMatchers("/listeVideogames") .hasAnyRole("ADMIN","AGENT","USER");
        httpSecurity.authorizeRequests().requestMatchers("/supprimerVideogame","/modifierVideogame","/updateVideogame") .hasAnyRole("ADMIN");
